...
from bson import errors as berror
...
def atualizar():
    ...
    try:
        ...
        print(f"Erro ao acessar ao banco de dados. {e}")
    except berror.InvalidId as f:
        print(f"ObjectID inválido. {f}")
...
def deletar():
    ...
    try:
        if db.produtos.count_documents({}) > 0:
            ...
            print("Não existem documentos para serem deletados.")
    except errors.PyMongoError as e:
        print(f"Erro ao acessar ao banco de dados. {e}")
    except berror.InvalidId as f:
        print(f"ObjectID inválido. {f}")
